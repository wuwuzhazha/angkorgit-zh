# merge-batches.ps1 — t5 合并器（round 2）：batches/{ui,web,rust}/batch.tsv 合并进 dict.tsv
# 规则：
#   - 与 dict 同 source（Trim 后精确相等，Ordinal）→ scope 并集 + 复用 dict target/anchor；target 不一致 → 报告并保留 dict。
#   - 批次间同 source → scope 并集；target 不一致 → 保留批次号小者并报告。
#   - source 命中 protect.txt → 允许（契约 §4.5/§6.3：引擎对含保护片段的 source 走 bypass 整串替换），仅报告 info。
#   - anchor 正则非法 / 单单词缺 anchor / 译文缺失 / scope 非法 → 丢弃并报告。
#   - 新增条目按首 scope 分节插入，节内按 source 长度降序（长串优先）；短未锚条目是长条目子串 → 移到长条目之后。
# 用法：pwsh -NoProfile -File zh-dict/merge-batches.ps1 [-Repo <repo根>]
param([string]$Repo = '')
$ErrorActionPreference = 'Stop'
if (-not $Repo) { $Repo = Join-Path $PSScriptRoot '..' }
$Repo = [System.IO.Path]::GetFullPath($Repo).TrimEnd('\', '/')
$zh = Join-Path $Repo 'zh-dict'
...
Rpt ("dict.tsv 换行符: " + $(if ($nl -eq "`r`n") { 'CRLF' } else { 'LF' }))

$sections = [ordered]@{
  'A. 桌面 UI（apps/desktop/src）'  = 'ui'
  'B. Rust 引擎错误消息'            = 'rust'
  'C. core 措辞'                    = 'core'
  'D. 文档'                         = 'docs'
  'E. 元数据'                       = 'meta'
...
# 反向：scope → 节名
$secOfScope = @{}
foreach ($k in $sections.Keys) { $secOfScope[$sections[$k]] = $k }

$items = New-Object System.Collections.Generic.List[object]
$curSection = ''
$baselineDupKeys = [System.Collections.Generic.Dictionary[string,int]]::new([System.StringComparer]::Ordinal)
$baselineCount = 0
foreach ($dl in [regex]::Split($rawDict, "\r\n|\n")) {
  $trim = $dl.Trim()
  if ($dl -match '^\s*#') {
    foreach ($k in $sections.Keys) {
      if ($dl -match [regex]::Escape($k)) { $curSection = $sections[$k]; break }
    }
...
Rpt ("dict.tsv 解析数据行: $baselineCount")

# ---------- 1. 保护正则（已升级的 protect.txt） ----------
$protect = @()
...
# ---------- 2. 读批次 ----------
$scopeTokenRe = '^(ui|rust|core|website|docs|meta|e2e|test)(,(ui|rust|core|website|docs|meta|e2e|test))*$'
$batchFiles = @(
...
$protectHit = New-Object System.Collections.Generic.List[string]
foreach ($bf in $batchFiles) {
  if (-not (Test-Path -LiteralPath $bf.P)) { Rpt "!! 缺批次文件: $($bf.P)"; continue }
  $raw = [System.IO.File]::ReadAllText($bf.P)
  ...
  foreach ($l in [regex]::Split($raw, "\r\n|\n")) {
    ...
    if ($why -ne '') { $dropped.Add("batch$($bf.N):$ln [$($c[1])] $why"); continue }
    $anchor = if ($c.Count -gt 3) { $c[3] } else { '' }
    if ($anchor -ne '') {
      try { $null = [regex]::new($anchor) } catch { $dropped.Add("batch$($bf.N):$ln [$($c[1])] anchor 正则非法: $anchor"); continue }
    }
    if (Test-Protect $c[1]) { $protectHit.Add("batch$($bf.N):$ln [$($c[1])]（引擎 bypass 整串替换，契约 §4.5）") }
    $scopes = @($c[0] -split ',' | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne '' })
    $batchEntries.Add([pscustomobject]@{ Batch = $bf.N; Line = $ln; Scopes = $scopes; Source = $c[1]; Target = $c[2]; Anchor = $anchor })
  }
}
Rpt ("批次数据行合计: $($batchEntries.Count)；丢弃: $($dropped.Count)")
foreach ($d in $dropped) { Rpt "  DROP $d" }
Rpt ("批次 source 命中 protect（bypass 放行，info）: $($protectHit.Count)")
foreach ($p in $protectHit) { Rpt "  PROTECT $p" }

# ---------- 3. 合并去重 ----------
$dictBySrc = [System.Collections.Generic.Dictionary[string,object]]::new([System.StringComparer]::Ordinal)
...
$seenBatch = [System.Collections.Generic.Dictionary[string,object]]::new([System.StringComparer]::Ordinal)
$newEntries = @{}
...
foreach ($e in $batchEntries) {
  ...
  if ($dictBySrc.ContainsKey($k)) {
    ...
    if ($di.Target -ne $e.Target) {
      $conflict.Add("batch$($e.Batch):$($e.Line) [$($e.Source)] target 与 dict 不一致（batch='$($e.Target)' dict='$($di.Target)'）→ 保留 dict")
      continue
    }
...
Rpt ("target 冲突保留: $($conflict.Count)")
foreach ($c2 in $conflict) { Rpt "  CONFLICT $c2" }
Rpt ("新增条目: $(($newEntries.Values | ForEach-Object { $_.Count } | Measure-Object -Sum).Sum)")

# ---------- 4. 分节插入（长串优先） ----------
$inserted = New-Object System.Collections.Generic.List[string]
foreach ($sec in ($secOfScope.Keys | ForEach-Object { $_ })) {
  ...
  foreach ($n in $sorted) {
    ...
    if ($insertAt -ge 0) {
      ...
      $inserted.Add("节[$sec] 前插 [$($n.Source)]（位于 [$($items[$insertAt + 1].Source)] 之前）")
    } else {
...
while ($iter -lt 100) {
  ...
  $moves.Add("scope[$($viol.S)] 短条目 [$($item.Source)] 移到长条目之后")
}
...
Rpt ("新增 (source+scope) 重复键: $($newDups.Count)")
...
foreach ($k in $globDefs.Keys) {
  ...
  $all = @(Get-ChildItem -LiteralPath $Repo -Recurse -Force -File | Where-Object { $_.FullName -notmatch '\\\.git\\|\\node_modules\\|\\target\\|\\dist\\' })
  foreach ($f in $all) {
    $rel = $f.FullName.Substring($Repo.Length + 1).Replace('\', '/')
    foreach ($r in $rx) { if ($r.IsMatch($rel)) { $found.Add($rel); break } }
  }
...
function Test-AnchorHit([string]$anchor, [string[]]$scopes) {
  ...
  foreach ($s in $scopes) {
    ...
    foreach ($rel in $scopeFileLists[$s]) {
      $abs = Join-Path $Repo ($rel.Replace('/', [System.IO.Path]::DirectorySeparatorChar))
      if (-not (Test-Path -LiteralPath $abs)) { continue }
      if (Select-String -Path $abs -Pattern $anchor -Quiet) { $hit = $true; break }
    }
    if ($hit) { break }
  }
  $hitCache[$key] = $hit
...
foreach ($l in $finalLines) {
  ...
  if (-not $batchSrcSet.Contains($c[1].Trim())) { continue }
  $scopes = @($c[0] -split ',' | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne '' })
  $checked++
  if (-not (Test-AnchorHit $c[3] $scopes)) { $misses.Add("[$($c[1])] anchor 未命中任何源文件: [$($c[3])] scope=$($c[0])") }
}
Rpt ("批次 anchor 命中校验：检查 $checked 条，未命中 $($misses.Count)")
...
Rpt "错误: $($errs.Count) | 冲突保留: $($conflict.Count) | 丢弃: $($dropped.Count) | 批次 anchor 未命中: $($misses.Count) | 排序违例: $($orderVio.Count) | 新增重复键: $($newDups.Count) | 批次 source 缺失: $($missingBatch.Count)"
$report | Write-Host
