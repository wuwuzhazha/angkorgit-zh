...
├── blame/BlamePanel.tsx      ← center view (ui.centerBlame {file, rev}; openBlame clears
│                               file history + diff, openFileHistory clears blame; Escape
│                               closes it; it counts as focusMode like file history so the
│                               inspector folds away per G36): header X / basename bold +
│                               dirname dimmed / "Working copy" or `at <short>` Badge with a
│                               "Back to working copy" button / File history button; body is
│                               a useVirtualizer list of 22px rows: a w-72 gutter that shows
│                               ONLY on the first line of each hunk (Avatar 16 + author +
│                               timeAgo + short hash, one role=button "Open commit <short>"
│                               that closes blame and revealCommit()s; hunk boundaries carry a
│                               border-t), line number, highlightLine code; hovering a row
│                               tints every row of the same commit; right-click menu: Open
│                               commit / Blame at this commit / Blame before this commit
│                               (rev `<oid>^`) / Copy hash, all disabled on the uncommitted
│                               hunk. ENTRY POINTS: DiffPanel header UserRoundSearch button
│                               (blames the commit the diff shows, else the working copy),
│                               working copy file menu "Blame", commit file menu "Blame at
│                               this commit" (sourceOid ?? commit oid). Demo: demoBlame
│                               synthesises hunks over demoConflictContent with the last
│                               hunk uncommitted
